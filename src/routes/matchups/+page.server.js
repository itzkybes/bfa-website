...
// (build-time import preferred; disk fallback attempted).
//
// Returns:
// {
...
// Attempt to load early2023.json from a few places on disk (returns parsed JSON or null)
...
export async function load(event) {
  ...
  // (We will push override-specific messages after clearing.)
  try {
    // clear only the prior debug messages to keep log focused on override
    messages.length = 0;
    ...
    if (isEarly2023) {
      let ovJson = null;
      // 1) Try build-time import (preferred). Path relative to this file:
      try {
        ...
        // 2) Try reading from disk candidates
        try {
          ovJson = await tryLoadEarlyJsonFromDisk(messages);
          if (ovJson) messages.push('Loaded early2023.json via disk fallback.');
        } catch (diskErr) {
          messages.push('Disk fallback while loading early2023.json failed: ' + (diskErr && diskErr.message ? diskErr.message : String(diskErr)));
        }
      }
