...
/**
 * Extract player points mapping from a participant object.
 * IMPORTANT: This implementation intentionally ONLY reads starter points
 * (e.g., `starters_points`, `startersPoints`, `starter_points`, etc.)
 * and maps player_id -> points for use when summing scores for W/L.
 *
 * Returns an object: { "<playerId>": <points>, ... }
 */
function extractPlayerPointsMap(participant) {
  const map = {};
  if (!participant || typeof participant !== 'object') return map;

  // prefer variants for starter points
  const sp = participant.starters_points ?? participant.startersPoints ?? participant.starter_points ?? participant.starterPoints ?? null;

  if (sp && typeof sp === 'object') {
    // If it's an object keyed by player id
    if (!Array.isArray(sp)) {
      for (const kk in sp) {
    ...
    // If it's an array, map against participant.starters (ordering matters)
    ...
    for (let si = 0; si < sp.length; si++) {
      const pts = safeNum(sp[si]);
      const pid = (starters && starters[si]) ? String(starters[si]) : null;
      if (pid) {
        map[pid] = pts;
      }
    }
  ...
  // If no starter-specific structure found, return empty map.
  // We intentionally do NOT inspect other non-starter fields here.
  return map;
}
...
export async function load(event) {
  ...
  // Attempt to load early overrides (static file) from same origin: /early2023.json
  let earlyOverrides = null;
  try {
    const earlyUrl = `${event.url.origin}/early2023.json`;
    try {
      const resp = await fetch(earlyUrl);
      if (resp && resp.ok) {
        const txt = await resp.text();
        try {
          earlyOverrides = JSON.parse(txt);
          messages.push('Loaded early2023.json overrides from ' + earlyUrl);
        } catch (parseErr) {
          earlyOverrides = null;
          messages.push('Fetch early2023.json threw: invalid json: ' + (parseErr?.message ?? String(parseErr)));
        }
      } else {
        messages.push('Fetching early2023.json returned status ' + (resp ? resp.status : 'no response'));
      }
    } catch (fetchErr) {
      earlyOverrides = null;
      messages.push('Fetch early2023.json failed: ' + (fetchErr?.message ?? String(fetchErr)));
    }
  } catch (e) {
    // protect runtime where event.url may not be present (shouldn't happen)
    earlyOverrides = null;
    messages.push('Unable to attempt early2023.json fetch: ' + (e?.message ?? String(e)));
  }
  ...
  for (let si = 0; si < seasons.length; si++) {
    ...
    try {
      ...
      let rosterMap = {};
      try { rosterMap = await sleeper.getRosterMapWithOwners(leagueId, { ttl: 60 * 5 }); } catch (e) { rosterMap = {}; messages.push('Failed to fetch rosterMap for league ' + leagueId + ' — ' + (e?.message ?? String(e))); }

      // build quick username -> rosterId & teamname -> rosterId maps for early override resolution
      ...
      if (rosterMap && Object.keys(rosterMap).length) {
        for (const rk in rosterMap) {
          ...
          if (meta.owner_name) teamNameToRoster[String(meta.owner_name).toLowerCase()] = String(rk);
        }
      }
      ...
      for (let week = 1; week <= MAX_WEEKS; week++) {
        // NOTE: If earlyOverrides contains entries for 2023 weeks 1..3, synthesize matchups here instead of calling the API.
        let matchups = null;
        let usedEarlyOverride = false;
        try {
          // check early override condition
          if (String(seasonEntry.season) === '2023' && week >= 1 && week <= 3 && earlyOverrides && earlyOverrides['2023'] && earlyOverrides['2023'][String(week)]) {
            const earlyList = earlyOverrides['2023'][String(week)] || [];
            const synthetic = [];
            // each earlyList item has { teamA: { name, ownerName }, teamB: {...}, teamAScore, teamBScore }
            for (let ei = 0; ei < earlyList.length; ei++) {
              const item = earlyList[ei];
              // determine roster ids for A & B by ownerName (username) first, then by team name
              let ownerA = (item.teamA && (item.teamA.ownerName ?? item.teamA.ownername ?? item.teamA.owner)) ? String(item.teamA.ownerName ?? item.teamA.ownername ?? item.teamA.owner) : null;
              let ownerB = (item.teamB && (item.teamB.ownerName ?? item.teamB.ownername ?? item.teamB.owner)) ? String(item.teamB.ownerName ?? item.teamB.ownername ?? item.teamB.owner) : null;
              let rosterA = null, rosterB = null;
              if (ownerA) rosterA = usernameToRoster[String(ownerA).toLowerCase()];
              if (!rosterA && item.teamA && item.teamA.name) rosterA = teamNameToRoster[String(item.teamA.name).toLowerCase()];
              if (!rosterA) rosterA = 'early:' + (ownerA || (item.teamA && item.teamA.name) || ('a' + ei));
              if (ownerB) rosterB = usernameToRoster[String(ownerB).toLowerCase()];
              if (!rosterB && item.teamB && item.teamB.name) rosterB = teamNameToRoster[String(item.teamB.name).toLowerCase()];
              if (!rosterB) rosterB = 'early:' + (ownerB || (item.teamB && item.teamB.name) || ('b' + ei));
              const mid = `early-2023-${week}-${ei}`;
              // push two participant-style entries that mirror the shape used downstream
              synthetic.push({
                matchup_id: mid,
                week: week,
                roster_id: rosterA,
                points: safeNum(item.teamAScore ?? item.teamA?.score ?? item.teamAScore)
              });
              synthetic.push({
                matchup_id: mid,
                week: week,
                roster_id: rosterB,
                points: safeNum(item.teamBScore ?? item.teamB?.score ?? item.teamBScore)
              });
            }
            matchups = synthetic;
            usedEarlyOverride = true;
            messages.push(`Applied early2023 override for season 2023 week ${week} (${(matchups || []).length} participant entries)`);
          } else {
            // no override, fetch real matchups
            try { matchups = await sleeper.getMatchupsForWeek(leagueId, week, { ttl: 60 * 5 }); } catch (mwErr) { matchups = null; /* fallthrough handled below */ }
          }
        } catch (mwErrOuter) {
          // if anything goes wrong, fallback to trying to get matchups via client
          try { matchups = await sleeper.getMatchupsForWeek(leagueId, week, { ttl: 60 * 5 }); } catch (mwErr) { matchups = []; messages.push('Failed to fetch matchups for league ' + leagueId + ' week ' + week + ' — ' + (mwErr?.message ?? String(mwErr))); }
        }
        ...
        for (let m = 0; m < mids.length; m++) {
          ...
          if (entries.length === 1) {
            ...
            // Compute points from starters if available, otherwise fallback
            const playerPointsOnly = extractPlayerPointsMap(only);
            let ptsOnly = 0;
            if (playerPointsOnly && Object.keys(playerPointsOnly).length) {
              for (const k in playerPointsOnly) if (Object.prototype.hasOwnProperty.call(playerPointsOnly, k)) ptsOnly += safeNum(playerPointsOnly[k]);
            } else {
              ptsOnly = safeNum(only.points ?? only.points_for ?? only.pts ?? 0);
            }
          ...
          // Build participants with rosterId, points (SUM OF STARTERS) , raw entry
          const participants = [];
          for (let p = 0; p < entries.length; p++) {
            const ent = entries[p];
            const pid = ent.roster_id ?? ent.rosterId ?? ent.owner_id ?? ent.ownerId;
            // prefer sum of starter points
            const playerPointsMap = extractPlayerPointsMap(ent);
            let ppts = 0;
            if (playerPointsMap && Object.keys(playerPointsMap).length) {
              for (const k in playerPointsMap) if (Object.prototype.hasOwnProperty.call(playerPointsMap, k)) ppts += safeNum(playerPointsMap[k]);
            } else {
              // fallback: use provided aggregate fields if present
              ppts = safeNum(ent.points ?? ent.points_for ?? ent.pts ?? 0);
            }
