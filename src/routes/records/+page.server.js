...
/**
 * Try to load static early2023.json:
 * (same approach as in matchups loader)
 */
async function loadEarlyJSON(event, messages) {
  try {
    if (event && event.url && event.url.origin) {
      const origin = event.url.origin;
      messages.push(`Attempting to load early2023.json from ${origin}/early2023.json`);
      try {
        const res = await fetch(`${origin}/early2023.json`);
        if (res.ok) {
          const txt = await res.text();
          try {
            const obj = JSON.parse(txt);
            messages.push('Fetched early2023.json via origin fetch.');
            return obj;
          } catch (pe) {
            messages.push('Fetch early2023 invalid json: ' + (pe && pe.message ? pe.message : String(pe)));
          }
        } else {
          messages.push('Fetch early2023.json failed with status: ' + res.status);
        }
      } catch (fe) {
        messages.push('Fetch early2023.json threw: ' + (fe && fe.message ? fe.message : String(fe)));
      }
    }
  } catch (e) {
    messages.push('Early JSON origin fetch attempt failed: ' + (e && e.message ? e.message : String(e)));
  }

  // Try to read from disk relative to this file
  try {
    const { readFile } = await import('fs/promises');
    const p = new URL('../../../static/early2023.json', import.meta.url);
    messages.push('Attempting to read static/early2023.json from disk: ' + p.pathname);
    try {
      const txt = await readFile(p, 'utf8');
      const obj = JSON.parse(txt);
      messages.push('Loaded early2023.json from disk.');
      return obj;
    } catch (re) {
      messages.push('Reading/parsing static/early2023.json failed: ' + (re && re.message ? re.message : String(re)));
    }
  } catch (e) {
    messages.push('Filesystem attempt to read early2023.json failed: ' + (e && e.message ? e.message : String(e)));
  }
...
export async function load(event) {
  ...
  // load early JSON once for all leagues
  var earlyJson = null;
  try {
    earlyJson = await loadEarlyJSON(event, messages);
  } catch (e) {
    messages.push('earlyJson load error: ' + (e && e.message ? e.message : String(e)));
    earlyJson = null;
  }
  ...
  for (var li = 0; li < leagueIdsToProcess.length; li++) {
    ...
    try {
      ...
      for (var week = 1; week <= MAX_WEEKS; week++) {
        ...
        // If early JSON override applies for this leagueSeason and this week (only 2023 weeks 1-3), build owner -> score map
        var earlyMapForThisWeek = null;
        try {
          if (earlyJson && leagueSeason != null) {
            var seasonKey = String(leagueSeason);
            if (earlyJson[seasonKey] && earlyJson[seasonKey][String(week)]) {
              earlyMapForThisWeek = {};
              const arr = earlyJson[seasonKey][String(week)];
              for (var ei = 0; ei < arr.length; ei++) {
                const matchObj = arr[ei];
                if (matchObj.teamA && matchObj.teamA.ownerName) earlyMapForThisWeek[String(matchObj.teamA.ownerName).toLowerCase()] = safeNum(matchObj.teamAScore);
                if (matchObj.teamB && matchObj.teamB.ownerName) earlyMapForThisWeek[String(matchObj.teamB.ownerName).toLowerCase()] = safeNum(matchObj.teamBScore);
              }
              messages.push('Using early2023 override for season=' + seasonKey + ' week=' + week);
            }
          }
        } catch (e) {
          messages.push('Error preparing early override map for week ' + week + ': ' + (e && e.message ? e.message : String(e)));
          earlyMapForThisWeek = null;
        }
        ...
        for (var mii = 0; mii < mids.length; mii++) {
          ...
          if (entries.length === 1) {
            ...
            // early override: find rosterId by owner name -> roster, then set ptsOnly accordingly
            if (earlyMapForThisWeek && rosterMap) {
              // try to map roster id -> owner_username/owner_name/team_name (lowercased)
              var metaOnly = rosterMap[String(ridOnly)] || {};
              var ownerKey0 = (metaOnly.owner_username || metaOnly.owner_name || metaOnly.team_name || '').toLowerCase();
              if (ownerKey0 && earlyMapForThisWeek[ownerKey0] != null) {
                ptsOnly = earlyMapForThisWeek[ownerKey0];
              }
            }
          ...
          for (var e = 0; e < entries.length; e++) {
            ...
            // early override: find owner key for pid then set ppts
            if (earlyMapForThisWeek && rosterMap) {
              var meta = rosterMap[String(pid)] || {};
              var ownerKey = (meta.owner_username || meta.owner_name || meta.team_name || '').toLowerCase();
              if (ownerKey && earlyMapForThisWeek[ownerKey] != null) {
                ppts = earlyMapForThisWeek[ownerKey];
              }
            }
