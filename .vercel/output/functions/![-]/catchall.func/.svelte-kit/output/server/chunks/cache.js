function createSleeperClient({ cache = null, concurrency = 8 } = {}) {
  const CACHE = cache || {
    async get() {
      return null;
    },
  ...
  async function rawFetch(path, opts = {}) {
    ...
    if (ttl && CACHE && typeof CACHE.set === "function") {
      try {
        await CACHE.set(cacheKey, j, ttl);
      } catch (e) {
      }
    }
    return j;
  ...
  async function getRosterMapWithOwners(leagueId, opts = {}) {
    ...
    if (ttl && CACHE && typeof CACHE.set === "function") {
      try {
        await CACHE.set(cacheKey, map, ttl);
      } catch (e) {
      }
    }
    return map;
      ...
      const participants = entries.map((en) => {
        ...
        if (roster_id && rosterMap && rosterMap[String(roster_id)]) meta = rosterMap[String(roster_id)];
          ...
          for (const rk of Object.keys(rosterMap)) {
            ...
            if (rmeta && rmeta.owner_id && String(rmeta.owner_id) === String(owner_id)) {
              meta = rmeta;
              break;
            }
          }
  ...
  return {
    ...
    // exported for backward-compatibility/tests
  };
}
...
function createKVCache(kvNamespace) {
  if (!kvNamespace) {
    throw new Error("createKVCache requires a kvNamespace object");
  }
  const hasGet = typeof kvNamespace.get === "function";
  typeof kvNamespace.put === "function" || typeof kvNamespace.set === "function" || typeof kvNamespace.setItem === "function";
  typeof kvNamespace.delete === "function" || typeof kvNamespace.del === "function" || typeof kvNamespace.remove === "function";
  const getFn = hasGet ? kvNamespace.get.bind(kvNamespace) : null;
  const putFn = typeof kvNamespace.put === "function" ? kvNamespace.put.bind(kvNamespace) : typeof kvNamespace.set === "function" ? kvNamespace.set.bind(kvNamespace) : null;
  const delFn = typeof kvNamespace.delete === "function" ? kvNamespace.delete.bind(kvNamespace) : typeof kvNamespace.del === "function" ? kvNamespace.del.bind(kvNamespace) : typeof kvNamespace.remove === "function" ? kvNamespace.remove.bind(kvNamespace) : null;
  if (!getFn || !putFn) {
...
export {
  createMemoryCache as a,
  createSleeperClient as b,
  createKVCache as c
};
