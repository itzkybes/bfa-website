...
var sleeper = createSleeperClient({ cache, concurrency: SLEEPER_CONCURRENCY });
...
Number(process.env.MAX_WEEKS) || 25;
async function load(event) {
  ...
  try {
    ...
    try {
      ...
      messages.push("Failed fetching base league: " + (e2 && e2.message ? e2.message : e2));
      console.error(e2);
    }
    if (mainLeague) {
      ...
      while (currPrev && steps < 50) {
        ...
        try {
          ...
          if (!prevLeague) {
            messages.push("Could not fetch previous_league_id " + currPrev);
            break;
          }
  ...
  if (selectedSeasonParam === "current" || selectedSeasonParam === "all") {
    ...
    for (var i = 0; i < seasons.length; i++) {
      if (String(seasons[i].league_id) === String(selectedSeasonParam)) {
        leagueIdToProcess = String(seasons[i].league_id);
        matched = true;
        break;
      }
    }
    if (!matched) {
      for (var j = 0; j < seasons.length; j++) {
        if (String(seasons[j].season) === String(selectedSeasonParam)) {
          leagueIdToProcess = String(seasons[j].league_id);
          matched = true;
          break;
        }
      }
  ...
  try {
    ...
    playersMap = {};
    messages.push("Failed to load players dataset: " + (e2 && e2.message ? e2.message : e2));
    console.error("rosters: players fetch failed", e2);
  }
  var resultLeague = {
  ...
  try {
    ...
    try {
      ...
      if (!rosterMap || Object.keys(rosterMap).length === 0) {
        ...
        if (Array.isArray(arr) && arr.length) {
          ...
          for (var ai = 0; ai < arr.length; ai++) {
            var e = arr[ai];
            if (e && e.roster_id != null) rosterMap[String(e.roster_id)] = e;
          }
          ...
          var rawUsers = [];
          try {
            rawRosters = await sleeper.getRosters(leagueIdToProcess, { ttl: CACHE_TTL }) || [];
          } catch (e2) {
            rawRosters = [];
            messages.push("raw getRosters failed: " + (e2 && e2.message ? e2.message : e2));
            console.error(e2);
          }
          try {
            rawUsers = await sleeper.getUsers(leagueIdToProcess, { ttl: CACHE_TTL }) || [];
          } catch (e2) {
            rawUsers = [];
            messages.push("raw getUsers failed: " + (e2 && e2.message ? e2.message : e2));
            console.error(e2);
          }
          var usersById = {};
          if (Array.isArray(rawUsers)) {
            for (var u = 0; u < rawUsers.length; u++) {
              var uu = rawUsers[u];
              var id = uu.user_id ?? uu.userId ?? uu.id;
              if (id != null) usersById[String(id)] = uu;
            }
          }
          if (Array.isArray(rawRosters)) {
            for (var rr = 0; rr < rawRosters.length; rr++) {
              ...
              var teamAvatar = null;
              try {
                if (rrr && rrr.metadata && rrr.metadata.team_avatar) teamAvatar = String(rrr.metadata.team_avatar).indexOf("http") === 0 ? String(rrr.metadata.team_avatar) : "https://sleepercdn.com/avatars/" + String(rrr.metadata.team_avatar);
              } catch (e2) {
              }
              ...
              if (uobj) {
                ...
                if (!ownerAvatar) try {
                  if (uobj.metadata && uobj.metadata.avatar) ownerAvatar = String(uobj.metadata.avatar).indexOf("http") === 0 ? String(uobj.metadata.avatar) : "https://sleepercdn.com/avatars/" + String(uobj.metadata.avatar);
                } catch (e2) {
                }
                if (!ownerAvatar) try {
                  if (uobj.avatar) ownerAvatar = String(uobj.avatar).indexOf("http") === 0 ? String(uobj.avatar) : "https://sleepercdn.com/avatars/" + String(uobj.avatar);
                } catch (e2) {
                }
              }
      ...
      messages.push("Error obtaining rosterMapWithOwners: " + (e2 && e2.message ? e2.message : e2));
      console.error(e2);
      rosterMap = {};
    }
    var normalized = [];
      ...
      for (var ridk in rosterMap) {
        ...
        if (playerIds && playerIds.length && playersMap) {
          for (var pi = 0; pi < playerIds.length; pi++) {
            ...
            var pobj = null;
            try {
              pobj = playersMap[pid] || playersMap[pid.toUpperCase()] || null;
            } catch (e2) {
              pobj = null;
            }
    ...
    if (!normalized || normalized.length === 0) {
      try {
        ...
        var usersById2 = {};
        for (var u2 = 0; u2 < rawUsers2.length; u2++) {
          var uu2 = rawUsers2[u2];
          var id2 = uu2.user_id ?? uu2.userId ?? uu2.id;
          if (id2 != null) usersById2[String(id2)] = uu2;
        }
        for (var rr2 = 0; rr2 < (rawRosters2 || []).length; rr2++) {
          ...
          var tav = null;
          try {
            if (rinfo && rinfo.metadata && rinfo.metadata.team_avatar) tav = String(rinfo.metadata.team_avatar).indexOf("http") === 0 ? String(rinfo.metadata.team_avatar) : "https://sleepercdn.com/avatars/" + String(rinfo.metadata.team_avatar);
          } catch (e2) {
          }
          var oav = null;
          if (uo) {
            try {
              if (uo.metadata && uo.metadata.team_avatar) oav = String(uo.metadata.team_avatar).indexOf("http") === 0 ? String(uo.metadata.team_avatar) : "https://sleepercdn.com/avatars/" + String(uo.metadata.team_avatar);
            } catch (e2) {
            }
            if (!oav) try {
              if (uo.metadata && uo.metadata.avatar) oav = String(uo.metadata.avatar).indexOf("http") === 0 ? String(uo.metadata.avatar) : "https://sleepercdn.com/avatars/" + String(uo.metadata.avatar);
            } catch (e2) {
            }
            if (!oav) try {
              if (uo.avatar) oav = String(uo.avatar).indexOf("http") === 0 ? String(uo.avatar) : "https://sleepercdn.com/avatars/" + String(uo.avatar);
            } catch (e2) {
            }
          }
        ...
        messages.push("Fallback raw rosters/users fetch failed: " + (e2 && e2.message ? e2.message : e2));
        console.error(e2);
      }
    }
