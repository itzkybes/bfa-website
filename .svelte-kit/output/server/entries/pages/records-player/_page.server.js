...
async function load(event) {
  ...
  const seasonsToProcess = seasons.length ? seasons.map((s) => ({ leagueId: s.league_id, season: s.season })) : [{ leagueId: BASE_LEAGUE_ID, season: (/* @__PURE__ */ new Date()).getFullYear() }];
  const playersMapGlobal = await fetchPlayersMap();
  const seasonsResults = [];
  for (const s of seasonsToProcess) {
    let computeStreaks = function(resultsArray) {
      ...
      return { maxW, maxL };
    }, buildStandingsFromTrackers = function(statsByRoster, resultsByRoster, paByRoster) {
      ...
      return out;
    }, findMatchForPair = function(rA, rB, preferredWeeks = [playoffStart, playoffStart + 1, playoffStart + 2]) {
      ...
      return null;
    }, decideWinnerFromMatch = function(matchRow, aId, bId) {
      ...
      const aPF = regularStandings.find((s2) => String(s2.rosterId) === a)?.pf ?? 0;
      const bPF = regularStandings.find((s2) => String(s2.rosterId) === b)?.pf ?? 0;
      if (aPF > bPF) return { winner: a, loser: b, reason: "tiebreak-pf" };
      if (bPF > aPF) return { winner: b, loser: a, reason: "tiebreak-pf" };
      const aW = regularStandings.find((s2) => String(s2.rosterId) === a)?.wins ?? 0;
      const bW = regularStandings.find((s2) => String(s2.rosterId) === b)?.wins ?? 0;
    ...
    }, runMatch = function(seedA, seedB, label, preferredWeeks = [playoffStart, playoffStart + 1, playoffStart + 2]) {
    ...
    if (!fullSeasonMatchupsRows || !fullSeasonMatchupsRows.length) {
      ...
      seasonsResults.push({ season: seasonLabel, leagueId, finalsMvp: null, overallMvp: null, teamLeaders: [], championshipWeek: playoffEnd, _sourceJson: localUsedKey ?? null, rosterMap });
      continue;
    }
    const wR1Pairs = [
      [1, 8],
  ...
  const allTimePlayoffMap = {};
  const allTimeFullMap = {};
  for (const sr of seasonsResults) {
    const sNum = Number(sr.season);
    if (isNaN(sNum) || sNum < 2022) continue;
